...
最近 90 天正文内嵌 PNG 走势图，不添加独立下载附件。仅展示人民币报价，每个点是一次成功查询的人民币最低参考报价（含起售价）；
观测点以折线连接，单条记录仅显示一个点，无报价显示空状态，邮件正文标记最新查询状态与时间。
邮件失败不影响价格保存，错误写入应用日志；当前不自动重试邮件。
航线的 `report_recipient` 字段可指定专属收件邮箱：设置后，该航线从默认日报中排除，
按专属收件人单独汇总发送机票走势图（不含汇率）；留空则继续使用 `EMAIL_TO`。
若航线收件人包含全部 `EMAIL_TO` 地址，该航线合并进默认日报，剩余收件人仍单独接收，避免默认收件人收到两封。
专属邮件同样受 `EMAIL_ENABLED` 控制，一个收件人发送失败不阻止其他收件人的邮件。
...
汇率采集现在明确请求北京时间当天的报价日期，并严格校验返回日期。
若当天尚未发布（包括返回旧日期或接口返回 404），不写入旧报价，邮件提示当天尚未发布，保留历史展示。
Frankfurter 是每日参考数据源，不能保证北京时间 09:00 或 09:30 已有当天报价；当前定时任务时间不变。

默认日报（`EMAIL_TO` 中最初两个收件人）同时包含人民币兑日元、人民币兑加元的近 90 天走势及双向换算。
两种汇率分别采集和保存状态，一种失败不阻断另一种；专属航线收件人的邮件保持仅含航班信息。
加元曲线纵轴为“加元 / 1 人民币”，越高表示同样人民币可兑换更多加元；仅展示已采集历史。
