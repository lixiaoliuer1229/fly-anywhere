...
将 `.env` 中 `EMAIL_ENABLED=true`，`EMAIL_TO` 设为收件邮箱。配置发件账号的
...
`SCRAPE_INTERVAL_HOURS=24` 表示服务启动 24 小时后开始、此后每 24 小时查询，
重启会重新计时，需保持服务运行。每轮全部航线查询结束后发送一封邮件，附各航线
最近 90 天 PNG 走势图。每个点是一次成功查询的最低参考报价（含起售价），按币种分图；
少于 8 次记录显示散点，无报价显示空状态，邮件正文标记最新查询状态与时间。
